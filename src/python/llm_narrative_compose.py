...
class LLMNarrativeScenesCompose(NarrativeScenesHandler):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        Parameters:
            ...
            Optional:
                - input_compose_filename (str): Path to file with scene specifications, required for composition
                - output_compose_filename (str): Path for saving composed scenes, required for composition
        ...
        self.lookback_scene_count_limit = 12 # code in methods should handle None in case this default is changed to None
        self.scene_compose_limit = None
        ...
        self.to_compose = None
        if self.input_compose_filename is not None and len(self.input_compose_filename) > 0:
    ...
    def _get_character_records(self, scene_index: int, character_list: list,
        ...
        if len(character_list) == 0:
            return {}
        prev_chron_scene_index = scene_index - 1
    ...
    def _get_setting_records(self, scene_index: int, setting_list: list,
    ...
    def _get_object_records(self, scene_index: int, object_list: list,
    ...
    def _get_scene_creation_descriptions(self, scene: dict) -> str:
        ...
        if len(character_list) > 0:
            char_dict = self._get_character_records(scene_index, character_list,
                                                    main_char_num_records, other_char_num_records)
        else:
            char_dict = {}
        if len(object_list) > 0:
            object_dict = self._get_object_records(scene_index, object_list,
                                                   first_object_num_records, other_object_num_records)
        else:
            object_dict = {}
        if len(setting_list) > 0:
            setting_dict = self._get_setting_records(scene_index, setting_list,
                                                     first_setting_num_records, other_setting_num_records)
        else:
            setting_dict = {}
    ...
    def _get_scene_creation_prompt_previous_narrative(self, scene: dict) -> str:
        ...
        if scene_remaining_len > 0:
            previous_narrative = self._get_scenes_text_for_characters_prior_to_scene(
                character_list, cur_scene_index, scene_remaining_len
                )
    ...
    def _get_scenes_text_for_characters_prior_to_scene(self, character_list: list[str], cur_scene_index: int,
                                                       char_count_limit: Optional[int] = None,) -> str:
        """
        ...
        if len(prior_scene_list) == 0:
            return ""
        if (self.lookback_scene_count_limit is not None and len(prior_scene_list) > self.lookback_scene_count_limit
            and self.lookback_scene_count_limit > 0):
            prior_scene_list = prior_scene_list[:self.lookback_scene_count_limit]
        ...
        # char_count_limit = kwargs.get('char_count_limit', None)
        text = ""
        for paragraph in paragraph_list_reversed:
            new_text_length = len(text) + len(paragraph) + 1
            if char_count_limit is not None and new_text_length > char_count_limit and char_count_limit > 0:
                break
            text = paragraph + "\n" + text

        return text

    def _get_scene_creation_prompt(self, scene: dict) -> str:
        """
        Generate the scene creation prompt.

        This method creates a structured prompt for generating a scene based on the provided scene specification.
        It includes contextual information, character descriptions, and other relevant details to guide the LLM.
        ...
        return introduction + recent_narrative + context + descriptions + first_paragraph

    def get_fine_tune_narrative_prompt_response_list(self) -> list[str]:
        """
        Fine-tune the scene text based on the LLM's response.

        This method is a placeholder for any additional processing or fine-tuning
        that may be required after generating the scene text.

        Required Parameters:
            - scene (dict): Scene specification to fine-tune

        Returns:
            None
        """
        ppr = self.preprocess_results
        train_scene_list = ppr.get_train_scene_list()
        if train_scene_list is None or len(train_scene_list) == 0:
            print("No train scene list found")
            return []
        prompt_response_pairs = []
        for scene in train_scene_list:
            prompt = self._get_scene_creation_prompt(scene)
            prompt_response_pairs.append((prompt, scene['body']))
    ...
    def _write_scene(self, scene: dict) -> None:
        ...
        scene_creation_request = self._get_scene_creation_prompt(scene)
    ...
    def write_scenes(self) -> None:
        ...
        if self.api_obj is None:
            raise ValueError("API object is not initialized. Please provide a valid model name.")
        if self.to_compose is None:
            raise ValueError("No scenes to compose. Please provide a valid scene to compose file.")
        if self.output_compose_filename is None:
            raise ValueError("No output compose filename provided. Please provide a valid output filename.")

        tc = self.to_compose

        for index, scene in enumerate(tc.scene_list):
            if self.scene_compose_limit is not None and index >= self.scene_compose_limit:
                if self.verbose:
